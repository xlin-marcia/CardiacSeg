...
#from networks import VisionTransformer as ViT_seg
#from networks import CONFIGS as CONFIGS_ViT_seg
...
def inference(args, model, test_save_path=None):
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        dice_scores, hd95_scores, iou_scores = test_single_volume(
            image, label, model, classes=args.num_classes, 
            patch_size=[args.img_size, args.img_size],
            test_save_path=test_save_path, case=case_name, z_spacing=args.z_spacing
        )
        metric_list += np.array([dice_scores, hd95_scores, iou_scores])
        logging.info('idx %d case %s mean_dice %f mean_hd95 %f mean_iou %f' % (
            i_batch, case_name, np.mean(dice_scores), np.mean(hd95_scores), np.mean(iou_scores)
        ))
